                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.programs.update', $program) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <select name="university_id" id="university_id" class="form-control">
                                    ...
                                    @foreach ($universities as $university)
                                        <option value="{{ $university->id }}" {{ old('university_id', $program->university_id) == $university->id ? 'selected' : '' }}>{{ $university->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="program_name" id="program_name" class="form-control" value="{{ old('program_name', $program->program_name) }}">
                                @error('program_name')<span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group">
                                <label for="field_of_study">Field of Study</label>
                                <input type="text" name="field_of_study" id="field_of_study" class="form-control" value="{{ old('field_of_study', $program->field_of_study) }}">
                                @error('field_of_study')<span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group">
                                <label for="tuition_fee_annual">Annual Tuition Fee</label>
                                <input type="number" step="0.01" min="0" name="tuition_fee_annual" id="tuition_fee_annual" class="form-control" value="{{ old('tuition_fee_annual', $program->tuition_fee_annual) }}">
                                @error('tuition_fee_annual')<span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group">
                                <label for="application_deadline">Application Deadline</label>
                                <input type="date" name="application_deadline" id="application_deadline" class="form-control" value="{{ old('application_deadline', $program->application_deadline) }}">
